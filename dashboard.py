...
from funzioni_dati import load_data, assegna_zona_custom
...
if check_password():
    ...
    if not df.empty:
        ...
        # 2. Applicazione Zone Cardio in base agli input manuali
        df['Zona Cardio'] = df['FC Media'].apply(
            lambda x: assegna_zona_custom(x, f["z1"], f["z2"], f["z3"], f["z4"])
        )
        ...
        if not df_f.empty:
            ...
            with tabs[3]:
                st.subheader("Performance per Zona Cardio (Valori Manuali)")
                # Grafico Box Plot per vedere il passo nelle zone inserite
